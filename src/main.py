...
if __name__ == "__main__":
    ...
    ticker1 = st.text_input('Введите тикер **базовой** валюты в чароноприёмник 😛', placeholder='e.g. BTC')
    ticker2 = st.text_input('Введите тикер **котируемой** валюты в чароноприёмник 😛', placeholder='e.g. USDT')
    pair = ticker1 + ticker2

    market_data = MarketData()
    pairs_list = market_data.get_possible_pairs()
    if pair in pairs_list:
        ...
        if timeframe:
            market_data.init_candle_df((ticker1, ticker2), time_dict[timeframe])
